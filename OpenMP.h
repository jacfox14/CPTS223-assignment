...
void calSum()
{
    ...
    #pragma omp parallel for reduction(+:sum)
        // todo
        for (int i = 0; i < data_size; i++) {
            sum += data[i];  // Each thread adds to the sum
        }
...
{
        // TODO
        int local_max = data[0];  // Each thread will have its own local_max

        // Split the work between threads
        #pragma omp for
        for (int i = 1; i < data_size; i++) {
            if (data[i] > local_max) {
                local_max = data[i];  // Each thread finds the max in its segment
            }
        }

        // Use a critical section to safely update the global max_value
        #pragma omp critical
        {
            if (local_max > max) {
                max = local_max;  // Update global max_value if needed
            }
        }
}
    // -------------- above TODO: OpenMP implementation --------------
